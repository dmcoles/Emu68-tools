...
namespace tinystd
{
    static inline void memset(void *to, char c, size_t n)
    {
        char *dst = reinterpret_cast<char *>(to);
        while(n--) *dst++ = c;
    }

    const char string::_null = 1;
    ...
    string::string(const char *src) : _buffer(NULL), _capacity(0), _length(0)
    {
        if (src)
        {
            ...
            resize_buffer(len + 1);
            _length = len;
            CopyMem((APTR)src, _buffer, len + 1);
        }
    }
    ...
    string &string::operator+=(const char *str)
    {
        ...
        CopyMem((APTR)str, _buffer + _length, len);
        _length += len - 1;
    ...
    string &string::append(const char *s, size_type n)
    {
        if (s != NULL)
        {
            ...
            CopyMem((APTR)s, _buffer + _length, len);
            _length += len;
        }
        return *this;
    }
    ...
    string &string::insert(size_type pos, const char *s)
    {
        if (s)
        {
            ...
            CopyMem((APTR)s, _buffer + pos, len);
    ...
    string &string::insert(size_type pos, size_type n, char c)
    {
        if (n)
        {
            ...
            CopyMem(_buffer + pos, _buffer + pos + len, _length - pos + 1);
            for (int i=0; i < len; i++) _buffer[pos + i] = c;
